...
In my opinion, yes, everyone can achieve this. Imagine trying to succeed in a subject like mathematics — you would have to compete with people who have dedicated their whole lives to it. However, when it comes to creating AI agents, you will only compete with those who have only a few months of experience in this field.

So the time is perfect, even if you are a beginner.
...
1. To use the most powerful tools for your agent, you will need some programming experience. You can connect agents to existing APIs, but in real conditions, we usually create proxy servers, even if we connect to a ready-made API. This allows us to better control the behavior of the tool.
...
Although SaaS platforms will undoubtedly develop and be able to meet some customer needs in the future, I believe that knowing code will still be necessary.

Moreover, with the advent of Cursor, the coding process has become like using another SaaS platform, since now you can write code using only natural language.
