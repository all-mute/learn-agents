...
Message: "Good day, please tell me about the features of your application."
```
</details>
...
```text
You are a support service assistant. Compose a polite and concise answer to the client based on the text of the email:
\"\"\"{email_body}\"\"\"
```
</details>
...
flowchart LR
    ...
    Intent -->|FAQ| FAQSearch[FAQ DB Search]
...
```text
You are an assistant that generates an answer based on the found FAQ.
...
<details>
<summary>Input Document Examples</summary>
...
```mermaid
flowchart LR
    Doc[Source document about 400,000 tokens] --> Split[Split by 6000 tokens]
...
For example, for a text of 18 megatokens, the algorithm will work in 2 summarization cycles:
...
```text
Document: Product Technical Specification (≈ 400,000 tokens)
```
...
```mermaid
flowchart LR
    U[User Processing Logic] --> Process
...
```text
You are an analytical model. Your task is to form a partial report based on the specified user logic and the input text chunk.
Input:
...
<summary>Code Review and Publication</summary>
  ...
  Your task is to, based on the Pull Request diff:
...
<summary>Marketing and Sales</summary>
  ...
  <summary>System Prompts</summary>
    ...
    ```text
    You are a copywriter assistant who creates personalized e-mails for each segment.
  ...
  <summary>System Prompts</summary>
    ...
    ```text
    You are a model that evaluates leads for readiness to buy.
    ...
    <details>
    <summary>LLM: Commercial Proposal</summary>

    ```text
    You are a sales assistant who generates commercial proposals.
    Input: lead {lead_id, company, need, budget} and CP template.
    Output — {lead_id, proposal: "text of commercial proposal"}
    ```
    </details>
    ...
    ```text
    You are an upsell assistant who forms an additional offer.
...
Task: for each point, determine whether it complies with the rules.
